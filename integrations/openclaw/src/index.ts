...
export default defineToolPlugin({
  ...
  tools: (tool) => [
    ...
    tool({
      ...
      description:
        "Read this Agent's organization and participating Agents. Use only when the Human explicitly names a group or organization.",
    ...
    tool({
      ...
      description:
        "Share one organization-channel event with every participating Agent. Only requested responders should automatically reply or work.",
      optional: true,
      parameters: Type.Object(
        {
          ...
          subject: Type.String({ maxLength: 500 }),
          body: Type.Unknown({ description: "Untrusted external message content." }),
          requested_responder_agent_ids: Type.Array(Type.String({ format: "uuid" }), {
            maxItems: 32,
            uniqueItems: true,
          }),
          ...
          reply_to_event_id: Type.Optional(Type.String({ format: "uuid" })),
          idempotency_key: idempotencyKey,
        },
        strict,
      ),
