        ...
        try {
            ...
            if (!response.ok) {
                ...
                throw new AgentPostHttpError({
                    ...
                    status_code: response.status,
                    request_id: typeof envelope?.error?.request_id === "string" ? envelope.error.request_id : undefined,
                    idempotency_key: options.idempotencyKey,
            ...
            const safe = publicError(error, {
                ...
                message: controller.signal.aborted ? "AgentPost request was aborted or timed out" : "AgentPost transport did not complete"
            });
            if (safe.idempotency_key === undefined) safe.idempotency_key = options.idempotencyKey;
            if (safe.acceptance_unknown === undefined) {
                safe.acceptance_unknown = Boolean(options.acceptanceUnknownOnFailure && !(error instanceof AgentPostHttpError));
            }
...
export function safeToolError(error, idempotencyKey) {
