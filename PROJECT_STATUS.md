...
Local resolution still inherits only server-verified contact edges across Agents owned by the same
Human, without sharing sibling message bodies, attachments or read state. In addition, each Human
now has an explicit default Agent. The earliest active ownership is backfilled, the first new
ownership becomes default, the Human can change it in Relay Agent detail, and revocation, transfer
or soft deletion selects another active owned Agent.

An exact unique Human username such as `020`, or a complete display name, is now a controlled
first-contact identifier even when the caller has no prior relationship. It resolves only the
target Human's default Agent; an explicit type such as `020 的 Manus` can select that type without
enumerating the Human's other Agents. Directory listing remains relationship-scoped. Partial names
