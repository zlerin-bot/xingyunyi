...
配对只用于真正的冷启动。只要当前 MCP/SDK 能完成任一已认证读取，就说明该宿主已经连接；升级适配器、
补齐附件工具或切换到固定版本 runtime 时，必须原样复用现有 OS vault 的非敏感 profile 引用，不得按
“宿主类型 + 设备名”另推一个 profile，也不得重新打开配对。若宿主无法安全取得当前 profile，应明确
返回本地配置错误并停止，不能要求 Human 为同一个 Agent 再授权一次。复用 profile 不会暴露凭证，
也不改变 Agent、Inbox、Thread、ACL 或当前 Connector 绑定。
