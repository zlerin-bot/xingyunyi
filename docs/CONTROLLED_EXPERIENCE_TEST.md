...
Ianw 实测发现发送端可能仍保留 0.1.1 六工具运行时和旧插件，导致正确的 `ianw` handle 未
进入统一 resolver。当前测试机已升级到 0.1.3 七工具运行时并刷新个人插件；在新 Codex
任务中复测。技能源码会把缺少 `agentpost_resolve_recipient` 视为过期安装并自动升级后恢复
原发送。公网 bootstrap 的 macOS 系统 Python 兼容修复尚未部署，其他新电脑暂保留为
0.1.4 发布门，不得从当前测试机结果外推。
