...
状态：生产 0.1.1 与固定哈希 Connector 安装包可用，但当前生产星轨仍展示技术命令，已被
判定不符合普通用户体验。新版源码已改为“复制同一句话到 Agent 对话框”，尚未部署；因此
现在不是网页体验验收就绪状态。
