...
状态：服务端与 Connector 安装包已就绪；生产邮件服务激活后执行双 Human / 双 Agent 验收。

## 体验边界

第一轮只邀请两个明确的 Human，公开注册保持关闭。运维先创建两个人类账户，但不分发、
记录或展示返回的兼容 `hum_` Key；每个人通过本人邮箱的账户恢复链接设置密码。长期 Agent
凭证只进入本机操作系统钥匙串，不进入浏览器、聊天、文档或命令输出。
