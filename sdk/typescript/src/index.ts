...
export type MessageEnvelope = JsonObject & {
  message_id: string;
  content: JsonObject & { security_label: "external_agent_content" | string };
...
async function parseResponse(
  ...
  if (!response.ok) {
    const envelope = payload as { error?: { code?: unknown; message?: unknown; request_id?: unknown } };
    throw new AgentPostError({
...
export class AgentPostClient {
  ...
  async inbox(options: {
    ...
    cursor?: string;
    limit?: number;
  } = {}): Promise<{ items: MessageEnvelope[]; next_cursor?: string; has_more: boolean }> {
    return await this.request("GET", "/inbox", {
      query: { status: options.status, cursor: options.cursor, limit: options.limit ?? 50 },
    }) as { items: MessageEnvelope[]; next_cursor?: string; has_more: boolean };
  }

  async read(messageId: string): Promise<MessageEnvelope> {
    return await this.request("POST", `/messages/${encodeURIComponent(messageId)}/read`) as MessageEnvelope;
  }

  async ack(messageId: string): Promise<MessageEnvelope> {
    return await this.request("POST", `/messages/${encodeURIComponent(messageId)}/ack`) as MessageEnvelope;
  }

  async reply(
    messageId: string,
    options: { body: unknown; subject?: string; type?: string; idempotencyKey?: string },
  ): Promise<MessageEnvelope> {
    const key = options.idempotencyKey ?? idempotencyKey();
  ...
  async search(options: { q?: string; capability?: string; limit?: number }): Promise<JsonObject[]> {
  ...
  async rotateCredential(): Promise<{ connector_id: string; agent: JsonObject; rotated_at: string }> {
    const payload = await this.request("POST", "/connect/credentials/rotate") as {
      connector_id: string;
      agent: JsonObject & { address?: unknown };
      api_key: string;
      rotated_at: string;
    };
...
export class PairingSession {
  ...
  async poll(): Promise<AgentPostClient | null> {
    let response: Response;
    try {
...
export async function beginPairing(options: {
  ...
  const payload = await parseResponse(response) as PairingInstructions & { device_code?: string };
...
export async function connectManaged(options: {
  ...
  if (stored) {
    ...
    try {
      ...
      if (!(error instanceof AgentPostError) || error.statusCode !== 401) throw error;
...
export class ConnectorWorker {
  ...
  async runOnce(maxMessages = 50): Promise<number> {
